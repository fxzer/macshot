...
class DetachedEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func show(image: NSImage, tool: AnnotationTool, color: NSColor, strokeWidth: CGFloat, annotations: [Annotation]) {
        ...
        let chromeH = EditorTopBarView.height + insetsBottom + safeMargin * 2
    ...
    private func enforceAspectFitVisibility(logicalSize: NSSize) {
        ...
        // We add an extra 24px (12px on each side) of safe breathing room.
        let safeMargin: CGFloat = 24.0

        let visibleWidth = scrollView.frame.width - scrollView.contentInsets.left - scrollView.contentInsets.right - safeMargin
        let visibleHeight = scrollView.frame.height - scrollView.contentInsets.top - scrollView.contentInsets.bottom - safeMargin
