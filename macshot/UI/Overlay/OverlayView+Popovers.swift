...
extension OverlayView {
    ...
    func showBeautifyPopover(anchorView: NSView? = nil, anchorRect: NSRect = .zero) {
        if PopoverHelper.isVisible {
            ...
            DispatchQueue.main.async { [weak self] in
                self?.showBeautifyPopover(anchorView: anchorView, anchorRect: anchorRect)
            }
            return
        }

        let picker = BeautifyPickerView(overlayView: self)
    ...
    func showEffectsPopover(anchorView: NSView? = nil, anchorRect: NSRect = .zero) {
        if PopoverHelper.isVisible {
            ...
            // 延迟打开新的popover，让旧的先完全关闭
            DispatchQueue.main.async { [weak self] in
                self?.showEffectsPopover(anchorView: anchorView, anchorRect: anchorRect)
            }
            return
        }
